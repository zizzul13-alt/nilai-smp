  ...
  async syncNamespace(authUserId: string, workspaceId: string): Promise<void> {
    ...
    try {
      ...
      for (const op of operations) {
        ...
        // FAILED and CONFLICT are durable manual-recovery states. They are never
        // automatic server work, but they causally block later work for the same entity.
        if (op.status === 'FAILED' || op.status === 'CONFLICT') {
          blockedEntities.add(entityKey);
          continue;
        }
        if (op.status !== 'PENDING_SAFE' || blockedEntities.has(entityKey)) continue;
        ...
        if (result.kind === 'saved') await markSavedAndMinimize(this.db, op.op_id);
          ...
          await markOperation(this.db, op.op_id, { status: 'CONFLICT', last_error_code: 'REVISION_CONFLICT' });
          blockedEntities.add(entityKey);
