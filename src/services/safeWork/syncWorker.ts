...
export class SafeWorkSyncWorker {
  ...
  async syncNamespace(authUserId: string, workspaceId: string): Promise<void> {
    ...
    try {
      const operations=(await pendingForNamespace(this.db,authUserId,workspaceId)).sort((a,b)=>a.created_at.localeCompare(b.created_at));const blocked=new Set<string>();
      for(const op of operations){if(op.auth_user_id!==authUserId||op.workspace_id!==workspaceId)continue;const key=op.causal_key||`${op.entity_type}:${op.entity_id}`;if(op.status==='FAILED'||op.status==='CONFLICT'){blocked.add(key);continue;}if(op.status!=='PENDING_SAFE'||blocked.has(key))continue;
        await markOperation(this.db,op.op_id,{attempt_count:op.attempt_count+1,last_attempt_at:new Date().toISOString()});const result=await this.apply(op);
        if(result.kind==='saved')await markSavedAndMinimize(this.db,op.op_id);
        else if(result.kind==='conflict'){await markOperation(this.db,op.op_id,{status:'CONFLICT',last_error_code:'REVISION_CONFLICT',conflict_snapshot:op.operation_kind==='assessment.judgement'?{canonical_state:result.canonical?.state??null,canonical_score:result.canonical?.score??null,canonical_revision:result.revision}:null});blocked.add(key);}
