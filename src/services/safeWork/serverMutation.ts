...
const permanentServerErrors:Record<string,string>={
  P3201:'WORKSPACE_INTEGRITY',P3202:'OP_ID_MISMATCH',P3203:'TARGET_NOT_OWNED_OR_FOUND',
  P3401:'WORKSPACE_INTEGRITY',P3402:'ASSESSMENT_NOT_OWNED_OR_FOUND',P3403:'ENROLLMENT_NOT_IN_CLASS',
  P3501:'WORKSPACE_INTEGRITY',P3502:'OP_ID_MISMATCH',P3503:'TARGET_NOT_OWNED_OR_FOUND',
  P3504:'TEACHING_CONTEXT_NOT_OWNED_OR_INVALID',P3505:'MEETING_NOT_IN_PROGRESS','22023':'INVALID_OPERATION',
};
...
function classifyError(error:{code?:string|null}):SyncResult|null{
  ...
  const code=permanentServerErrors[error.code??''];
  return code?{kind:'failed',code}:null;
}
...
export async function applyMeetingCheckpoint(client:SupabaseClient,op:PendingOperation):Promise<SyncResult>{
  const p=op.payload as MeetingCheckpointPayload;
  const{data,error}=await client.rpc('apply_meeting_checkpoint_operation',{p_op_id:op.op_id,p_meeting_id:p.meeting_id,p_stopped_at:p.stopped_at,p_next_step:p.next_step});
  if(error)return classifyError(error)??{kind:'failed',code:error.code||'SERVER_ERROR'};
  const row=Array.isArray(data)?data[0]:data;
  if(!row)return{kind:'failed',code:'EMPTY_SERVER_RESULT'};
