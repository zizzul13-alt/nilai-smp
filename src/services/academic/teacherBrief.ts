...
function stale(recordedAt:string|null,now:Date){
  if(!recordedAt)return false;
  const d=new Date(now.getFullYear(),now.getMonth(),now.getDate());
  const mondayOffset=(d.getDay()+6)%7;d.setDate(d.getDate()-mondayOffset-7);
  return new Date(recordedAt)<d;
}
...
export function deriveTeacherBriefContext(input:{now:Date;today:TodayServerSnapshot;safe:TeacherBriefSafeSummary;results:ResultStateRow[];pacing:PacingRow[];reporting:ReportingRow[]}):TeacherBriefContext{
  ...
  const continuity_attention=input.today.classes.flatMap(row=>{
    if(row.active_meeting_id&&!row.active_checkpoint_id)return[{class_id:row.class_id,class_name:row.class_name,reason:'NO_CHECKPOINT' as const}];
    if(row.effective_stopped_at&&stale(row.effective_recorded_at,input.now))return[{class_id:row.class_id,class_name:row.class_name,reason:'STALE_CONTEXT' as const}];
    return[];
  });
...
export async function loadTeacherBriefContext(client:SupabaseClient,workspaceId:string,safe:TeacherBriefSafeSummary,now=new Date()):Promise<TeacherBriefContext>{
  const[today,resultQ,pacingQ,reportQ]=await Promise.all([
    loadTodayServer(client),
    client.from('assessment_results').select('state').eq('workspace_id',workspaceId),
    client.from('lesson_pacing_plans').select('normal_meetings,available_meetings,correction_reserve').eq('workspace_id',workspaceId),
    client.from('reporting_cycles').select('status').eq('workspace_id',workspaceId),
  ]);
...
export async function narrateTeacherBrief(context:TeacherBriefContext,narrator?:TeacherBriefNarrator):Promise<TeacherBriefNarrative>{
  if(!narrator)return deterministicTeacherBrief(context);
  try{const result=await narrator(context);return{headline:result.headline,priorities:result.priorities,source:'provider'};}
