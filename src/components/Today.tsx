...
export function Today({client,userId,workspaceId,onOpenContinuity,onOpenRapid}:Props){
  ...
  async function saveBaseline(){
    ...
    try{
      ...
      setNotice(editor.kind==='START_FROM_TODAY'?'Baseline baru disimpan. Riwayat lama tetap dipertahankan.':'Pembaruan cepat disimpan sebagai baseline baru; riwayat lama tidak diubah.');setEditor(null);baselineAttempt.current=null;
      try{const snapshot=await loadTodayServer(client);setState(current=>current.status==='ready'?{...current,snapshot}:current);}catch{setNotice('Baseline tersimpan di server. Halaman Hari ini belum dapat dimuat ulang; jangan anggap penulisan gagal.');}
  ...
  async function openRecovery(){
    ...
    if(firstRecovery.operation_kind==='meeting.checkpoint'){
      try{const classId=await resolveMeetingClass(client,workspaceId,firstRecovery.entity_id);if(!classId){setNotice('Kelas untuk checkpoint ini belum dapat ditentukan.');return;}setNotice('');onOpenContinuity(classId);}
      catch{setNotice('Kelas untuk checkpoint ini belum dapat ditentukan.');}
