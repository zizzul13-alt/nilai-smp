...
import type{AssessmentJudgementPayload}from'../domain/safeWork';
...
import{classifyReentryAge,deriveTodayModel,loadTodayServer,recordContinuityBaseline,type ReentryKind,type TodayClassContext,type TodayServerSnapshot}from'../services/academic/today';
...
export function Today({client,userId,workspaceId,onOpenContinuity,onOpenRapid}:Props){
  ...
  function openRecovery(){
    ...
    if(firstRecovery.operation_kind==='meeting.checkpoint'){
      const context=classes.find(c=>c.active_meeting_id===firstRecovery.entity_id||c.latest_actual_meeting_id===firstRecovery.entity_id);onOpenContinuity(context?.class_id);
  ...
  return<section className="today-shell">
    ...
    <section className="today-section today-now"><h2>NOW</h2>
      {primary?.kind==='continue-class'&&primaryClass?<><strong>{primaryClass.class_name} · Meeting aktif</strong><div className="today-memory"><div><small>LAST</small><b>{primaryClass.effective_stopped_at??'Belum ada checkpoint'}</b></div><div><small>NEXT</small><b>{primaryClass.effective_next_step??'Belum dicatat'}</b></div></div>{primaryClass.active_lesson_title?<p className="muted">Lesson: {primaryClass.active_lesson_title}</p>:null}</>:null}
      ...
      {primary?<button type="button" className="today-primary" onClick={runPrimary}>{primary.label}</button>:<p><strong>Tidak ada work yang perlu perhatian.</strong> Tidak ada jadwal yang perlu dikonfigurasi agar Today tetap aman.</p>}
