...
export function DailyDriverSetup({client,workspaceId,onReady}:Props){
  ...
  return<section className="setup-shell">
    ...
    <div className="setup-grid">
      <section className="setup-card"><h2>1 · Tahun ajaran</h2><input value={year} onChange={e=>setYear(e.target.value)} placeholder="2026/2027"/><button disabled={busy||!year.trim()} onClick={()=>void run(()=>createAcademicYearForSetup(client,workspaceId,year,context.years.length+1),()=>setYear(''),'Tahun ajaran tersimpan.')}>Tambah tahun</button></section>
      <section className="setup-card"><h2>2 · Periode</h2><select value={selectedYear} onChange={e=>setSelectedYear(e.target.value)}>{context.years.map(x=><option key={x.id} value={x.id}>{x.display_name}</option>)}</select><input value={period} onChange={e=>setPeriod(e.target.value)} placeholder="Semester 1"/><button disabled={busy||!selectedYear||!period.trim()} onClick={()=>void run(()=>createAcademicPeriodForSetup(client,workspaceId,selectedYear,period,context.periods.filter(x=>x.academic_year_id===selectedYear).length+1),()=>setPeriod(''),'Periode tersimpan.')}>Tambah periode</button></section>
      <section className="setup-card"><h2>3 · Kelas</h2><select value={selectedPeriod} onChange={e=>setSelectedPeriod(e.target.value)}>{context.periods.map(x=><option key={x.id} value={x.id}>{x.display_name}</option>)}</select><input value={className} onChange={e=>setClassName(e.target.value)} placeholder="VIII A"/><button disabled={busy||!selectedPeriod||!className.trim()} onClick={()=>void run(()=>createClassForSetup(client,workspaceId,selectedPeriod,className),()=>setClassName(''),'Kelas tersimpan.')}>Tambah kelas</button></section>
