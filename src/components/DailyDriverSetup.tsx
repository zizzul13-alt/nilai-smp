...
export function DailyDriverSetup({client,workspaceId,onReady}:Props){
  ...
  const roster=context.enrollments.filter(x=>x.class_id===selectedClass).map(enrollment=>context.students.find(studentRow=>studentRow.id===enrollment.student_id)).filter((studentRow):studentRow is NonNullable<typeof studentRow>=>Boolean(studentRow)).sort((a,b)=>a.display_name.localeCompare(b.display_name,'id'));
  return<section className="setup-shell">
    ...
    {context.classes.length?<section className="roster-browser" aria-labelledby="roster-title"><div><p className="eyebrow">Data yang sudah ada</p><h2 id="roster-title">Kelas & siswa</h2><p className="muted">Pilih kelas untuk memeriksa roster tanpa mengubah data.</p></div><label className="field-label">Kelas<select value={selectedClass} onChange={e=>setSelectedClass(e.target.value)}>{context.classes.map(x=><option key={x.id} value={x.id}>{x.display_name}</option>)}</select></label><div className="roster-summary"><strong>{context.classes.find(x=>x.id===selectedClass)?.display_name??'Kelas'}</strong><span>{roster.length} siswa</span></div>{roster.length?<ol className="roster-list">{roster.map(studentRow=><li key={studentRow.id}><span>{studentRow.display_name}</span><small>{studentRow.nis??studentRow.nisn??'Tanpa NIS/NISN'}</small></li>)}</ol>:<p className="empty-state" role="status">Kelas ini belum memiliki siswa.</p>}</section>:null}
