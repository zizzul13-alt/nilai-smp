...
export function DailyDriverSetup({client,workspaceId,onReady}:Props){
  ...
  return<section className="setup-shell">
    <header><p className="eyebrow">Data & pengaturan</p><h1>Siapkan kelas untuk dipakai sehari-hari</h1><p className="muted">Buat hanya data yang benar-benar ada. Jadwal tidak dibuat otomatis dan pengaturan ini tidak pernah membuat pertemuan.</p></header>
    <div className="setup-progress"><strong>{ready?'Siap mengajar':'Belum lengkap'}</strong><span>{context.classes.length} kelas · {context.students.length} siswa · {context.enrollments.length} keanggotaan kelas · {context.lessons.length} pelajaran</span></div>
    ...
    <div className="setup-grid">
      ...
      <section className="setup-card"><h2>5 · Keanggotaan kelas</h2><select value={selectedStudent} onChange={e=>setSelectedStudent(e.target.value)}><option value="">Pilih siswa</option>{context.students.map(x=><option key={x.id} value={x.id}>{x.display_name}</option>)}</select><select value={selectedClass} onChange={e=>setSelectedClass(e.target.value)}><option value="">Pilih kelas</option>{context.classes.map(x=><option key={x.id} value={x.id}>{x.display_name}</option>)}</select><button disabled={busy||!selectedStudent||!selectedClass} onClick={()=>void run(()=>createEnrollmentForSetup(client,workspaceId,selectedStudent,selectedClass),()=>{},'Keanggotaan kelas tersimpan.')}>Masukkan ke kelas</button></section>
      <section className="setup-card"><h2>6 · Materi & pelajaran</h2><input value={material} onChange={e=>setMaterial(e.target.value)} placeholder="Materi, mis. Gaya"/><button disabled={busy||!material.trim()} onClick={()=>void run(()=>createMaterialForSetup(client,workspaceId,material),()=>setMaterial(''),'Materi tersimpan.')}>Tambah materi</button><select value={selectedMaterial} onChange={e=>setSelectedMaterial(e.target.value)}><option value="">Pilih materi</option>{context.materials.map(x=><option key={x.id} value={x.id}>{x.title}</option>)}</select><input value={lesson} onChange={e=>setLesson(e.target.value)} placeholder="Pelajaran, mis. Gesekan"/><button disabled={busy||!selectedMaterial||!lesson.trim()} onClick={()=>void run(()=>createLessonForSetup(client,workspaceId,selectedMaterial,lesson),()=>setLesson(''),'Pelajaran tersimpan.')}>Tambah pelajaran</button></section>
    </div>
    {message?<p className="work-message" role="status">{message}</p>:null}
    {ready&&onReady?<button type="button" className="continue-primary" onClick={onReady}>Kembali ke Hari ini</button>:null}
  </section>;
}
