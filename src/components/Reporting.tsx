...
export function Reporting({client,workspaceId}:Props){
  ...
  async function savePolicy(){
    ...
    if(policyDraft.name.trim()===''||kkm!==null&&!Number.isFinite(kkm)){setNotice({kind:'error',text:'Nama kebijakan wajib dan KKM harus angka atau kosong.'});return;}
    ...
    try{
      ...
      setPolicyId(result.policy_id);setPolicyDraft(null);setNotice({kind:'info',text:`Kebijakan laporan v${result.version_no} dibuat. Formula tetap SIMPLE_MEAN dan kebijakan tersimpan sebagai versi baru.`});
  ...
  async function snapshotAction(finalize:boolean){
    ...
    try{
      ...
      if(result.outcome==='conflict'){
        setNotice({kind:'error',text:'Laporan berubah di tempat lain. Data terbaru dimuat; cek ulang sebelum mencoba lagi.'});
        await refreshCycle(actionClassId);
        return;
      }
      await refreshCycle(actionClassId);
      if(classIdRef.current!==actionClassId)return;
      setNotice({kind:'info',text:finalize?'Final — snapshot rapor ditutup secara eksplisit. Koreksi berikutnya wajib dibuka kembali dulu.':'Snapshot sementara dibuat dari Result kanonik saat ini.'});
    }catch(error){if(classIdRef.current===actionClassId)setNotice({kind:'error',text:error instanceof Error?error.message:String(error)});}finally{setBusy(false);}
  }
  ...
  async function reopen(){
    ...
    if(reopenReason.trim()===''){setNotice({kind:'error',text:'Alasan membuka kembali wajib supaya koreksi faktual dapat diaudit.'});return;}
    setBusy(true);setNotice(null);
    try{
      ...
      if(classIdRef.current!==actionClassId)return;
      if(result.outcome==='conflict'){setNotice({kind:'error',text:'Laporan berubah di tempat lain. Data terbaru dimuat.'});await refreshCycle(actionClassId);return;}
      setReopenReason('');await refreshCycle(actionClassId);
      if(classIdRef.current!==actionClassId)return;
      setNotice({kind:'info',text:'Siklus laporan dibuka kembali. Snapshot FINALIZED lama tetap utuh sebagai riwayat; buat snapshot sementara/final baru setelah koreksi.'});
    }catch(error){if(classIdRef.current===actionClassId)setNotice({kind:'error',text:error instanceof Error?error.message:String(error)});}finally{setBusy(false);}
  }

  if(!context)return<section className="continuity-shell"><p>Memuat laporan…</p>{notice?<p role="alert">{notice.text}</p>:null}</section>;
  return<section className="continuity-shell reporting-shell">
    <header><p className="eyebrow">Laporan · hasil akademik kanonik</p><h1>Laporkan dengan benar, lalu tutup secara eksplisit</h1><label className="field-label">Kelas<select value={classId} onChange={e=>switchClass(e.target.value)}>{context.classes.length===0?<option value="">Belum ada Kelas aktif</option>:null}{context.classes.map(c=><option key={c.id} value={c.id}>{c.display_name}</option>)}</select></label></header>
    {!selectedClass?<div className="continuity-empty"><strong>Belum ada Kelas aktif.</strong></div>:<>
      <div className="checkpoint-card"><h2>Kebijakan Laporan</h2>{policies.length?<><label className="field-label">Kebijakan<select value={policyId} disabled={busy} onChange={e=>setPolicyId(e.target.value)}>{policies.map(p=><option key={p.id} value={p.id}>{p.name} · v{p.version_no}</option>)}</select></label>{selectedPolicy?<p className="muted">SIMPLE_MEAN · Missing {selectedPolicy.missing_policy} · Remedial CURRENT_RESULT · Pembulatan {selectedPolicy.rounding_mode} · KKM {selectedPolicy.kkm??'—'}</p>:null}<button type="button" className="secondary" disabled={busy} onClick={createNewPolicyVersion}>Buat versi kebijakan baru</button></>:<p>Belum ada kebijakan laporan untuk periode Kelas ini.</p>}<button type="button" className="secondary" disabled={busy} onClick={()=>setPolicyDraft(blankPolicy())}>Kebijakan baru</button></div>
      {policyDraft?<div className="checkpoint-card"><h2>{policyDraft.policyKey?'Versi kebijakan baru':'Kebijakan baru'}</h2><label className="field-label">Nama<input value={policyDraft.name} onChange={e=>setPolicyDraft({...policyDraft,name:e.target.value})}/></label><label className="field-label">Missing<select value={policyDraft.missingPolicy} onChange={e=>setPolicyDraft({...policyDraft,missingPolicy:e.target.value as PolicyDraft['missingPolicy']})}><option value="EXCLUDE">EXCLUDE</option><option value="ZERO">ZERO</option></select></label><p className="muted">Remedial: CURRENT_RESULT. Attempt REMEDIAL mentah tetap menjadi bukti dan tidak otomatis dipromosikan menjadi nilai rapor.</p><label className="field-label">Pembulatan<select value={policyDraft.roundingMode} onChange={e=>setPolicyDraft({...policyDraft,roundingMode:e.target.value as PolicyDraft['roundingMode']})}><option value="NONE">NONE</option><option value="INTEGER">INTEGER</option><option value="ONE_DECIMAL">ONE_DECIMAL</option></select></label><label className="field-label">KKM (terpisah dari formula)<input inputMode="decimal" value={policyDraft.kkm} onChange={e=>setPolicyDraft({...policyDraft,kkm:e.target.value})} placeholder="opsional"/></label><div className="today-actions"><button type="button" disabled={busy} onClick={()=>void savePolicy()}>Simpan versi kebijakan</button><button type="button" className="secondary" disabled={busy} onClick={()=>setPolicyDraft(null)}>Batal</button></div></div>:null}
      {selectedPolicy?<div className="checkpoint-card"><div className="continuity-status"><strong>Siklus laporan</strong><span>{cycle?.status??'OPEN'}</span></div><p className="muted">Finalisasi hanya boleh lewat tindakan eksplisit. UNCHECKED memblok finalisasi; MISSING dan EXCUSED tetap state nyata dan diperlakukan oleh kebijakan.</p>{cycle?.status==='FINALIZED'?<><label className="field-label">Alasan membuka kembali<input value={reopenReason} onChange={e=>setReopenReason(e.target.value)} placeholder="contoh: koreksi nilai siswa setelah bukti ditemukan"/></label><button type="button" disabled={busy} onClick={()=>void reopen()}>Buka kembali untuk koreksi faktual</button></>:<div className="today-actions"><button type="button" className="secondary" disabled={busy} onClick={()=>void snapshotAction(false)}>Pratinjau sementara</button><button type="button" disabled={busy} onClick={()=>void snapshotAction(true)}>Finalisasi laporan</button></div>}</div>:null}
      {snapshot?<div className="checkpoint-card"><div className="continuity-status"><strong>Snapshot #{snapshot.snapshot_no}</strong><span>{snapshot.kind}</span></div><p className="muted">{snapshot.assessment_count} penilaian · {snapshot.enrollment_count} enrollment · snapshot append-only.</p><div className="report-table" role="table"><div className="report-row report-row--head" role="row"><strong>Siswa</strong><strong>Nilai</strong><strong>State bukti</strong><strong>KKM</strong></div>{rows.map(row=><div className="report-row" role="row" key={row.id}><span>{row.student_display_name}</span><strong>{formatReportedScore(row.reported_score)}</strong><span>{row.graded_count} GRADED · {row.missing_count} MISSING · {row.excused_count} EXCUSED · {row.unchecked_count} UNCHECKED</span><span>{row.meets_kkm===null?'—':row.meets_kkm?'LULUS':'DI BAWAH'}</span></div>)}</div></div>:null}
    </>}
