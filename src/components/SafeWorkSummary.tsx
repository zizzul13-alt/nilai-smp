...
export function SafeWorkSummary({userId,workspaceId,onOpen}:Props){
  ...
  if(!counts.pending&&!counts.attention)return<span className="safe-summary safe-summary--saved" role="status">Tidak ada antrean lokal</span>;
  return<button type="button" className={`safe-summary ${counts.attention?'safe-summary--attention':'safe-summary--pending'}`} onClick={onOpen}>{counts.attention?`${counts.attention} perlu tindakan`:''}{counts.attention&&counts.pending?' · ':''}{counts.pending?`${counts.pending} tertunda lokal`:''}</button>;
}
