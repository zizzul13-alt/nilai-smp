...
import{deterministicTeacherBrief,loadTeacherBriefContext,type TeacherBriefContext,type TeacherBriefNarrative}from'../services/academic/teacherBrief';
...
export function TeacherBrief({client,userId,workspaceId}:{client:SupabaseClient;userId:string;workspaceId:string}){
  ...
  const refresh=useCallback(async()=>{
    ...
    try{
      ...
      setState({status:'ready',context,narrative:deterministicTeacherBrief(context)});
  ...
  return<section className="today-shell teacher-brief-shell">
    <header><p className="eyebrow">F2 · Brief Guru</p><h1>{narrative.headline}</h1><p className="muted">Ringkasan deterministik dari data kanonik. Tidak mengubah nilai, Meeting, laporan, atau dokumen. AI/provider bersifat opsional dan tidak diperlukan agar brief ini bekerja.</p></header>
    ...
    <p className="muted">Dibuat {new Date(context.generated_at).toLocaleString('id-ID')} · sumber narasi: {narrative.source==='deterministic'?'deterministik':'provider opsional'}.</p>
  </section>;
}
