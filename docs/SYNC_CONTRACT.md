...
`syncNamespace(auth_user_id, workspace_id)` owns drain correctness. There is at most one active runner for each Safe Work namespace. A sync request that arrives while the same namespace is running joins that runner and marks a coalesced rerun request instead of being dropped or starting a parallel same-namespace worker. Different namespaces keep independent runner state, so a busy workspace cannot swallow another workspace's wakeup.

After each pass, the runner checks for newly eligible durable `PENDING_SAFE` work that was not already attempted during the current overall sync request. This lets work enqueued mid-flight and causal successors exposed by a predecessor becoming Saved drain without an unrelated later trigger. Per-causal-key blocking remains unchanged: an unsaved predecessor, FAILED row, or CONFLICT row prevents a later operation on the same causal key from leapfrogging it.

Retries remain bounded. An op_id is attempted at most once per overall sync request. A network/auth retryable result therefore remains truthfully `PENDING_SAFE` and waits for a later explicit startup/reconnect/manual sync trigger; coalesced wakeups do not immediately hammer the same failed operation. FAILED, CONFLICT, and `LEGACY_ATTEMPT_KIND_UNCERTAIN` rows are never auto-reactivated by the drain loop. Lost-ACK retries keep the original op_id and payload.
