...
expect_value 'historical evidence remains reconstructable after Assessment changes profile' "$A select string_agg(a.sequence_no||':'||p.config->>'wrong',',' order by a.sequence_no) from public.assessment_attempts a left join public.scoring_profiles p on p.workspace_id=a.workspace_id and p.id=a.scoring_profile_id where a.result_id=(select id from public.assessment_results where assessment_id='$ASSESS' and enrollment_id='$ENROLL');" '1:-5,2:-5,3:-5,4:-5,5:-10'
