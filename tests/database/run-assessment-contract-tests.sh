...
PROFILE="91000000-0000-0000-0000-000000000001"; ASSESS="92000000-0000-0000-0000-000000000001"; ENROLL="50000000-0000-0000-0000-000000000001"
...
expect_value 'MAKEUP evidence is distinct and original remains' "$A select state from public.record_assessment_judgement('$ASSESS','$ENROLL','GRADED',7,'MAKEUP',7,'{}'); select string_agg(attempt_kind,',' order by sequence_no) from public.assessment_attempts where result_id=(select id from public.assessment_results where assessment_id='$ASSESS' and enrollment_id='$ENROLL');" 'ORIGINAL,CORRECTION,MAKEUP'
expect_value 'REMEDIAL evidence is distinct from MAKEUP' "$A select state from public.record_assessment_judgement('$ASSESS','$ENROLL','GRADED',8,'REMEDIAL',8,'{}'); select string_agg(attempt_kind,',' order by sequence_no) from public.assessment_attempts where result_id=(select id from public.assessment_results where assessment_id='$ASSESS' and enrollment_id='$ENROLL');" 'ORIGINAL,CORRECTION,MAKEUP,REMEDIAL'
...
# B fixtures for deterministic cross-workspace attacks. Prior cumulative suite created B year/period/class.
BSTUDENT="93000000-0000-0000-0000-000000000001"; BENROLL="94000000-0000-0000-0000-000000000001"; BPROFILE="95000000-0000-0000-0000-000000000001"; BACT="96000000-0000-0000-0000-000000000001"
run "$B insert into public.students(id,workspace_id,display_name) values('$BSTUDENT',$BW,'B Student'); insert into public.enrollments(id,workspace_id,student_id,class_id) values('$BENROLL',$BW,'$BSTUDENT','83000000-0000-0000-0000-000000000001'); insert into public.scoring_profiles(id,workspace_id,name) values('$BPROFILE',$BW,'B Profile'); insert into public.activities(id,workspace_id,class_id,title) values('$BACT',$BW,'83000000-0000-0000-0000-000000000001','B Activity');"
...
expect_fail 'foreign user cannot mutate A Assessment' "$B update public.assessments set title='stolen' where id='$ASSESS'; select 1/0 where exists(select 1 from public.assessments where id='$ASSESS');"
