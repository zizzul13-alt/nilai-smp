...
"${PSQL[@]}" -f supabase/migrations/202609050003_continuity_write_boundary.sql >/dev/null
...
expect_value 'Start Class RPC succeeds for owner' "$A select outcome||':'||meeting_status||':'||replayed from public.start_teaching_meeting_operation('$START1','$CLASS',null,null);" 'started:in_progress:false'
...
expect_fail 'authenticated owner direct INSERT Meeting denied' "$A insert into public.meetings(workspace_id,class_id,occurred_at,status) values($AW,'$CLASS',now(),'in_progress');"
expect_fail 'authenticated owner direct UPDATE Meeting denied' "$A update public.meetings set status='completed' where id='$M1';"
expect_fail 'authenticated owner direct DELETE Meeting denied' "$A delete from public.meetings where id='$M1';"
expect_value 'Meeting remains in progress after denied direct mutations' "$A select status from public.meetings where id='$M1';" 'in_progress'

expect_value 'checkpoint RPC succeeds for owner and sequence starts at one' "$A select outcome||':'||sequence_no||':'||replayed from public.apply_meeting_checkpoint_operation('$CP1','$M1','Halaman 37, contoh gaya gesek nomor 2','Bahas nomor 3 lalu latihan mandiri');" 'saved:1:false'
CPID="$(run "$A select id from public.checkpoints where meeting_id='$M1' and sequence_no=1;")"
expect_fail 'authenticated owner direct INSERT Checkpoint denied' "$A insert into public.checkpoints(workspace_id,meeting_id,sequence_no,stopped_at) values($AW,'$M1',99,'bypass');"
expect_fail 'authenticated owner direct UPDATE Checkpoint denied' "$A update public.checkpoints set stopped_at='rewritten history' where id='$CPID';"
expect_fail 'authenticated owner direct DELETE Checkpoint denied' "$A delete from public.checkpoints where id='$CPID';"
...
expect_value 'explicit Complete Meeting RPC succeeds' "$A select outcome||':'||meeting_status||':'||replayed from public.set_teaching_meeting_status_operation('$FINISH1','$M1','completed');" 'saved:completed:false'
...
expect_value 'explicit Cancel Meeting RPC succeeds and is preserved as history' "$A select meeting_status from public.set_teaching_meeting_status_operation('$CANCEL2','$M2','cancelled');" 'cancelled'
...
expect_fail 'direct competing Meeting remains denied at privilege boundary' "$A insert into public.meetings(workspace_id,class_id,occurred_at,status) values($AW,'$CLASS',now(),'in_progress');"
...
expect_value 'schema version remains continuity core after repair migration' "$A select version from public.app_schema_version where id=1;" 'r3.4-continuity-core.1'

printf '\nR3.4 Teaching Continuity PostgreSQL repair matrix completed successfully.\n'
