-- R3.5-02 Governor repairs.
-- Qualify revision writes, close archive/reservation races, and preserve exact-once replay semantics.
...
declare
  ...
  art public.artifacts;
  new_version public.artifact_versions;
...
begin
  ...
  update public.artifacts as a
    set current_version_id=new_version.id, revision=a.revision+1, updated_at=now()
    where a.id=p_artifact_id and a.workspace_id=owned_workspace_id
    returning a.* into art;

  insert into public.audit_events(workspace_id,actor_user_id,event_type,entity_type,entity_id,metadata)
  values(owned_workspace_id,caller_id,'artifact.version.appended','artifact',p_artifact_id,jsonb_build_object('version_id',new_version.id,'version_no',next_no,'revision',art.revision));
  ...
  values(p_op_id,owned_workspace_id,'artifact.version-append','artifact',p_artifact_id,request_meta,art.revision,jsonb_build_object('version_id',new_version.id,'version_no',next_no));
...
begin
  ...
  select ao.* into prior from public.applied_operations ao where ao.op_id=p_op_id;
  if found then
    if prior.workspace_id<>owned_workspace_id or prior.operation_type<>'artifact.archive' or prior.target_entity_id<>p_artifact_id or prior.request_metadata<>request_meta then raise exception 'op_id scope or payload mismatch' using errcode='P3202'; end if;
  ...
  update public.artifacts as a
    set status='archived', revision=a.revision+1, updated_at=now()
    where a.id=p_artifact_id and a.workspace_id=owned_workspace_id
    returning a.* into art;

  insert into public.audit_events(workspace_id,actor_user_id,event_type,entity_type,entity_id,metadata)
  values(owned_workspace_id,caller_id,'artifact.archived','artifact',p_artifact_id,jsonb_build_object('revision',art.revision));
  insert into public.applied_operations(op_id,workspace_id,operation_type,target_entity_type,target_entity_id,request_metadata,result_revision,result_metadata)
  values(p_op_id,owned_workspace_id,'artifact.archive','artifact',p_artifact_id,request_meta,art.revision,'{}'::jsonb);
...
declare
  ...
  owned_workspace_id uuid;
  art public.artifacts;
  obj public.artifact_objects;
  prior public.applied_operations;
  request_meta jsonb;
  extension text;
  normalized_mime text:=btrim(coalesce(p_mime_type,''));
begin
  if caller_id is null then raise exception 'authentication required' using errcode='28000'; end if;
  ...
  select ao.* into prior from public.applied_operations ao where ao.op_id=p_op_id;
  if found then
    if prior.workspace_id<>owned_workspace_id or prior.operation_type<>'artifact.object-reserve' or prior.target_entity_id<>p_artifact_version_id or prior.request_metadata<>request_meta then raise exception 'op_id scope or payload mismatch' using errcode='P3202'; end if;
    return query select 'saved'::text,(prior.result_metadata->>'object_id')::uuid,prior.result_metadata->>'storage_path',true; return;
  end if;

  -- Same lock family as append/archive: no reservation can cross an archive boundary.
  perform pg_advisory_xact_lock(hashtextextended(owned_workspace_id::text||':artifact:'||p_artifact_id::text,0));
  select ao.* into prior from public.applied_operations ao where ao.op_id=p_op_id;
  if found then
    if prior.workspace_id<>owned_workspace_id or prior.operation_type<>'artifact.object-reserve' or prior.target_entity_id<>p_artifact_version_id or prior.request_metadata<>request_meta then raise exception 'op_id scope or payload mismatch' using errcode='P3202'; end if;
  ...
  values(owned_workspace_id,p_artifact_id,p_artifact_version_id,p_object_kind,'PENDING_UPLOAD',owned_workspace_id::text||'/'||p_artifact_id::text||'/'||p_artifact_version_id::text||'/'||p_op_id::text||extension,normalized_mime,p_byte_size,caller_id)
  returning * into obj;

  insert into public.audit_events(workspace_id,actor_user_id,event_type,entity_type,entity_id,metadata)
  values(owned_workspace_id,caller_id,'artifact.object.reserved','artifact_object',obj.id,jsonb_build_object('artifact_id',p_artifact_id,'version_id',p_artifact_version_id,'kind',p_object_kind));
  insert into public.applied_operations(op_id,workspace_id,operation_type,target_entity_type,target_entity_id,request_metadata,result_revision,result_metadata)
  values(p_op_id,owned_workspace_id,'artifact.object-reserve','artifact_version',p_artifact_version_id,request_meta,1,jsonb_build_object('object_id',obj.id,'storage_path',obj.storage_path));
...
values(1,'r3.5-artifact-core.3',now())
