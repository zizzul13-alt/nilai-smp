...
create table public.applied_operations (op_id uuid primary key,workspace_id uuid not null references public.workspaces(id) on delete restrict,operation_type text not null check(btrim(operation_type)<>''),target_entity_type text not null check(btrim(target_entity_type)<>''),target_entity_id uuid not null,request_metadata jsonb not null,result_revision bigint not null check(result_revision>=1),result_metadata jsonb not null default '{}'::jsonb,applied_at timestamptz not null default now(),constraint applied_operation_workspace_op_unique unique(workspace_id,op_id));
...
revoke all on public.applied_operations from anon; grant select on public.applied_operations to authenticated;
create policy applied_operation_owner_select on public.applied_operations for select to authenticated using(exists(select 1 from public.workspaces w where w.id=applied_operations.workspace_id and w.owner_user_id=auth.uid()));
...
begin
 ...
 update public.students as s set display_name=normalized_name,revision=s.revision+1,updated_at=now() where s.id=p_student_id and s.workspace_id=owned_workspace_id returning s.revision into current_revision;
 insert into public.applied_operations(op_id,workspace_id,operation_type,target_entity_type,target_entity_id,request_metadata,result_revision,result_metadata) values(p_op_id,owned_workspace_id,'student.rename','student',p_student_id,request_meta,current_revision,jsonb_build_object('display_name',normalized_name));
 return query select 'saved'::text,current_revision,false;
end;$$;
revoke all on function public.apply_student_rename_operation(uuid,uuid,text,bigint) from public,anon; grant execute on function public.apply_student_rename_operation(uuid,uuid,text,bigint) to authenticated;
insert into public.app_schema_version(id,version,applied_at) values(1,'r3.2-safe-work.1',now()) on conflict(id) do update set version=excluded.version,applied_at=excluded.applied_at;
