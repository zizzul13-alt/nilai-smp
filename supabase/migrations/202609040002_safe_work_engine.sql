...
create or replace function public.apply_student_rename_operation(
  p_op_id uuid, p_student_id uuid, p_display_name text, p_expected_revision bigint
)
...
begin
  ...
  if btrim(p_display_name) = '' then raise exception 'student name required' using errcode = '22023'; end if;
  ...
  if found then
    if prior.workspace_id <> owned_workspace_id or prior.operation_type <> 'student.rename' or prior.target_entity_id <> p_student_id then
      raise exception 'op_id scope mismatch' using errcode = '42501';
    end if;
  ...
  select s.revision into current_revision from public.students s
    where s.id = p_student_id and s.workspace_id = owned_workspace_id for update;
  ...
  update public.students set display_name = btrim(p_display_name), revision = revision + 1, updated_at = now()
    where id = p_student_id and workspace_id = owned_workspace_id
    returning students.revision into current_revision;

  insert into public.applied_operations(op_id, workspace_id, operation_type, target_entity_type, target_entity_id, result_revision, result_metadata)
  values (p_op_id, owned_workspace_id, 'student.rename', 'student', p_student_id, current_revision,
